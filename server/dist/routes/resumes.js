...
const prisma_1 = require("../lib/prisma"); // Need to create this
const zod_1 = require("zod");
const router = (0, express_1.Router)();
// Schema matching frontend types
const resumeSchema = zod_1.z.object({
    title: zod_1.z.string(),
    personalInfo: zod_1.z.any(), // Store as JSON
    summary: zod_1.z.string().optional(),
    experience: zod_1.z.any(), // Store as JSON
    education: zod_1.z.any(), // Store as JSON
    ...
    try {
        ...
        const resume = await prisma_1.prisma.resume.create({
            data: {
                ...
                title: data.title,
                content: data, // Storing unstructured JSON for flexibility
            },
        });
        res.json(resume);
    }
...
router.delete("/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.query.userId; // Optional security check
...
router.put("/:id", async (req, res) => {
    ...
    try {
        ...
        const data = resumeSchema.parse(req.body);
        const resume = await prisma_1.prisma.resume.update({
            where: { id },
            data: {
                title: data.title,
                content: data,
